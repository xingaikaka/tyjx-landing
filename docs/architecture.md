...
> ⚠️ **2026-05 架构变更**:中转层已从 Cloudflare Worker 迁移到 VPS Node 中转层
> (`packages/relay-server` → 部署为 `tyjx-portal-server`,VPS:3020)。
> 本文中所有提到 "Worker / wrangler / KV" 的章节都已弃用,请参阅
> [`packages/relay-server/README.md`](../packages/relay-server/README.md)。
> 新链路:`用户 → cdn666 → VPS:80 → Nginx → relay-server:3020 → 入口/发布 SSR`。
> admin → relay 配置走 `/api/portal/runtime`(AES-CBC iv:cipher),不再用 KV。
...
```
        ...
        ┌─────────────────────────────────────────────────────┐
        │         tyjx.app(品牌固定域,1 个)                  │
        │         CF Worker:host 命中 → 302                    │
        └────────────────────┬────────────────────────────────┘
                             │ 302 → 随机选 entryPages 池一个
                             ▼
        ┌─────────────────────────────────────────────────────┐
        │  入口页面(图 1)                                    │
        │  CF Worker,host ∈ entryPages[]                      │
...
| 层级 | 字段 | 数量 | 部署 |
|---|---|---|---|
| **品牌门户域** | `brandDomains[]` | 1+ | CF DNS → Worker。`brandDomains[0]` 是主品牌域(用于文案展示),其余作备用。当前只配 1 个 `tyjx.app`,以后可在 admin 后台加备用域 |
| **入口页面泛域** | `entryPages[]` | N(2~10) | CF DNS → Worker(每个域 + 通配符) |
| **发布页面泛域** | `publishPages[]` | N(3~20) | CF DNS → Worker(每个域 + 通配符) |
| **真落地页泛域** | `finalLandings[]` | N(3~10) | CF DNS A → 43.128.4.201 + 通配符 SSL |
...
#### 公开(供 Worker / luodiye_video 拉,不需登录)

```
GET  /api/portal/runtime              Worker 拉:domains + portalUI(AES-CBC 加密)
...
## 6. 加密通信(Worker ↔ admin-server)

- 共享 secret = `PORTAL_API_SECRET`(放 wrangler secrets / admin-server `.env`)
- 协议 = AES-256-CBC(沿用 `_legacy/functions/_shared/crypto.js` 兼容前端 CryptoJS)
- 请求带 `X-Timestamp` 防重放(±5 分钟)
...
```js
const route = (host) => {
  if (domains.brandDomains.includes(host))   return 'redirect-to-entry';
  if (matchPool(host, domains.entryPages))   return 'render-entry-page';
  if (matchPool(host, domains.publishPages)) return 'render-publish-page';
  return '404';
};

// matchPool: host === pool[i] || host endsWith "." + pool[i]
// 因为入口/发布页用泛域,实际访问的是 <随机 10 字符>.tyjxn3k8m2p7vc.cc 这种子域
function matchPool(host, pool) {
  return pool.some((d) => host === d || host.endsWith('.' + d));
}
```

KV 缓存 30 秒,失败时用上一次缓存,再失败用 wrangler.toml 兜底常量。
...
```
        ...
        │    *.<entryPages>.cc → CF         │
        │    *.<publishPages>.cc → CF       │
        │    *.<finalLandings>.cc → A 43.128.4.201 │
        └──────────────────────────────────┘
        ...
        │    server admin-portal.xxx:       │
        │      → :3010(server) + :3011(web) │
        │                                   │
        │  PM2:                             │
        │    admin-portal-server  (port 3010)│
        │    admin-portal-web     (port 3011)│
        │                                   │
        │  数据:                            │
        │    /www/tyjx-landing/packages/    │
        │      admin-server/src/data/       │
        │        portal.db (SQLite)         │
        │        uploads/ (媒体)            │
        └──────────────────────────────────┘
```
...
Phase 5  tyjx-portal Worker 开发 + 部署           可立刻并行(本地)
