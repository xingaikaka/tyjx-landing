...
export default function LandingPage() {
  ...
  const [apkSlug, setApkSlug] = useState('tianya');
  ...
  async function handleApkUpload(file: File) {
    ...
    setApkProgress(0);
    try {
      const r = await apkApi.upload(file, apkSlug || undefined, (p) => setApkProgress(p));
      const purgeStatus = r.purge?.ok
        ? `,CDN 缓存已请求刷新`
        : r.purge?.msg
          ? `,CDN 刷新失败:${r.purge.msg}`
          : '';
      toast.success(`APK 上传成功 (${fmtSize(r.size)})${purgeStatus}`);
      // 固定地址方案下 url 与 shortLink 同值,直接采用
      const useUrl = r.shortLink || r.url;
      setData((prev) => ({
        ...prev,
        androidApkUrl: useUrl,
  ...
  async function handleApkPurge(id: number) {
    try {
      const r = await apkApi.purge(id);
      if (r.ok) {
        toast.success(`已请求腾讯云 CDN 刷新${r.taskId ? `(task: ${r.taskId})` : ''}`);
      } else {
        toast.error(`刷新失败:${r.msg || '未知错误'}`);
      }
    } catch (e) {
      ...
      <div className="card">
            ...
            <MediaPicker
              ...
              onUploadDone={({ url, posterUrl }) => {
                setData((prev) => ({
                  ...prev,
                  backgroundVideo: url,
                  // 每次上传新 mp4 → 总用新视频首帧覆盖 poster。
                  // 用户想用自定义封面,可上传完之后单独在 poster 字段再换。
                  backgroundVideoPoster: posterUrl || '',
                }));
              }}
              previewHeight={120}
            />
        ...
        >
          ...
          <div style={{ fontSize: 12, color: 'var(--text-3)', marginBottom: 10 }}>
            ...
            <br />
            <b style={{ color: 'var(--text-2)' }}>稳定短链:</b>
            上传时填一个 slug(默认 <code>app</code>),会得到一个永久不变的下载 URL,
            例如 <code>/d/app.bin</code>。同 slug 后续覆盖上传 = "发新版本",
            短链自动指向最新文件,旧版本 R2 文件保留(可回滚)。
            把短链贴 openinstall / 海报二维码,后续不必再改第三方平台。
          </div>
          ...
          <div className="form-row">
            ...
            <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
              ...
              <span style={{ fontSize: 12, color: 'var(--text-3)' }}>
                R2 路径:<code>downloads/{apkSlug || 'tianya'}.bin</code> · 同 slug 上传 = 覆盖,URL 永不变
              </span>
            </div>
          </div>
          ...
          {apkList.length > 0 && (
            <div className="form-row">
              ...
              >
                {apkList.map((a) => {
                  const inUse = data.androidApkUrl === a.url || data.androidApkUrl === a.shortLink;
                  return (
                    <div
                      key={a.id}
                      style={{
                    ...
                    >
                      ...
                      <button
                        ...
                        title="复制固定下载地址(粘贴到 openinstall 等)"
                      >
                        复制地址
                      </button>
                      <button
                        type="button"
                        onClick={() => handleApkPurge(a.id)}
                        title="手动调腾讯云 CDN 刷新此地址"
                      >
                        刷新 CDN
                      </button>
                      <button
                        ...
                        title="使用此 APK 作为落地页 Android 下载地址"
                      >
                        使用
                      </button>
                      <button
